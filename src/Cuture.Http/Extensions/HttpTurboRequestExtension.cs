...
namespace Cuture.Http
{
    ...
    public static class HttpTurboRequestExtension
    {
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IHttpTurboRequest ToHttpRequest(this string url)
        {
            if (url.Length == 0)
            {
                throw new ArgumentException("it is null or empty", nameof(url));
            }
            return new HttpTurboRequest(url);
        }
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IHttpTurboRequest ToHttpRequest(this Uri uri)
        {
            return new HttpTurboRequest(uri);
        }
