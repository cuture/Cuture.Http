...
namespace Cuture.Http.Test.Internal
{
    ...
    public class SpanExtensionsTest
    {
        ...
        public void TruncationOneElementTest(string source, char indentifier, string targetFront, string targetBack)
        {
        ...
        public void TruncationElementsTest(string source, string indentifier, string targetFront, string targetBack)
        {
            var sourceSpan = source.AsSpan();
            var @default = new string(sourceSpan.TruncationStart(indentifier));
            Assert.AreEqual(targetBack, @default);
